...
import { verifyOtpFn } from "#/features/verification/data/mutations/functions/verify-otp";
...
export function OtpInput({ email, onSuccess, onBack }: OtpInputProps) {
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		setError(null);
		try {
			const { token } = await verifyOtpFn({ data: { email, code } });
			onSuccess(token);
		} catch (err) {
	...
	return (
		<form
			...
			className="flex h-full grow w-full flex-col gap-6"
		>
